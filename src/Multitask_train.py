...
def train(task):
    if (task == 'design'):
        ...
        model_name = 'inceptionv4_design'
    else:
        task_list = task_list_length
        model_name = 'inceptionv4_length'
    ...
    gen_train = Generator(X_train, y_train, batch_size=15, aug=True)
    ...
    model2.fit_generator(gen_train.generator, steps_per_epoch=gen_train.steps, epochs=3,
                         validation_data=(X_valid, y_valid))

    model2.compile(optimizer=Adam(0.000025), loss='categorical_crossentropy', metrics=[acc])
    model2.fit_generator(gen_train.generator, steps_per_epoch=gen_train.steps, epochs=2,
                         validation_data=(X_valid, y_valid))
    ...
    model2.fit_generator(gen_train.generator, steps_per_epoch=gen_train.steps, epochs=1,
                         validation_data=(X_valid, y_valid))
